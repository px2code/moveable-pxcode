import { IS_WEBKIT, IS_WEBKIT605, PREFIX, TINY_NUM } from "./consts";
import { InvertObject, prefixNames } from "framework-utils";
import {
    document,
    getRad,
    getShapeDirection,
...
export function getCSSSize(target: SVGElement | HTMLElement, iframeSelector: string) {
  const iframe = iframeSelector ? document.querySelector(iframeSelector) as HTMLIFrameElement : null;
  const contentWindow = iframe ? iframe.contentWindow : window;
...
export function fillEndParams<T extends IObject<any>>(
  moveable: any,
  e: any,
  params: Pick<T, Exclude<keyof T,
    "moveable" | "target" | "clientX" | "clientY" | "inputEvent" |
    "datas" | "currentTarget" | "lastEvent" | "isDrag" | "isDouble">> & { isDrag?: boolean },
): T {
  const datas = e.datas;
